...
load("@rules_rust//rust/private:per_crate_flag_trim.bzl", "CARGO_CONTEXT_SETTING", "CRATE_SETTINGS", "trim_crate_settings")
...
def _settings(context = ""):
    return {
        ...
        CARGO_CONTEXT_SETTING: context,
    }

def _generated_context_normalization_impl(ctx):
    env = unittest.begin(ctx)
    settings = _settings(_MACOS)
    attr = struct(cargo_contexts = {_MACOS: _LINUX, _LINUX: _LINUX})
...
def _workspace_preserves_cargo_settings_impl(ctx):
    env = unittest.begin(ctx)
    settings = _settings(_MACOS)
    asserts.equals(env, settings, trim_crate_settings(settings, struct()))
    asserts.equals(env, settings, trim_crate_settings(settings, struct(cargo_contexts = {})))
    asserts.equals(env, settings, trim_crate_settings(settings, struct(cargo_contexts = {_LINUX: ""})))
    return unittest.end(env)

def _build_script_enters_execution_context_impl(ctx):
    env = unittest.begin(ctx)
    attr = struct(cargo_contexts = {"": _LINUX})
