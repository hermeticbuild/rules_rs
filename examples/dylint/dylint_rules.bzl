...
def _dylint_transition_impl(_settings, _attr):
    ...
    # checked target graph must be compiled by nightly rustc with the compiler
    # internals exposed through the generated sysroot. Transition only the
    # explicit Dylint subtree; ordinary Rust builds keep the caller's normal
    # toolchain selection and never fetch `rustc-dev`.
    return {
        "@rules_rust//rust/toolchain/channel:channel": "nightly",
        "@rules_rs//rs/toolchains/rustc_dev:enabled": True,
    }
...
def dylint_toolchain(
        ...
        *,
        ...
        visibility = None,
        tags = []):
    """Declares one Dylint driver toolchain for nightly rustc-dev builds."""
...
def _rust_dylint_action(ctx, crate_info, config, renamed_libraries):
    ...
    dylint_toolchain = ctx.toolchains[str(Label("//:toolchain_type"))]
